...
class serverresp implements Runnable
{
	client parenobjt;
	int myport=0;
	
	serverresp(client obj,int port)
	{
		parenobjt=obj;
		myport=port;
		...
		try
		{
			...
			while (true)
			{
				...
				if (status.equals("SUCCESS"))
				{
					...
					//System.out.println(" downloaded Successfully. Total Time Taken 2 Dwnld file = "+ttkn+" ms");
				...
				else
				{
					long end=System.currentTimeMillis();
					long ttkn=end-parenobjt.start;
					parenobjt.jta.append("File Name(Not Found)::"+fname+"Time Taken::"+ttkn+"ms\n");
